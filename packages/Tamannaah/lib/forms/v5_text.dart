...
class V5Text<T> extends FormBuilderField<T> {
  //-----------------------------
  //
  ...
  V5Text({
    ...
    this.mouseCursor,
        ...
        super(
          key: key,
          initialValue: cast<T>(initialValue), //,controller != null ? controller.text : initialValue,
...
//---------------------------
          ...
          onChanged: ((value) {
            ...
            //     cast<T>(valueTransformer<T>(valTransformer ?? ((v) => cast<T>(v)), mask)(controller.text)) !=
...
//---------------------------
          ...
          builder: (FormFieldState<T?> field) {
            final state = field as _V5TextState<T>;
                        ...
                        child: Row(
                          ...
                          children: [
                            obscure == null
                                ...
                                : IconButton(
                                    icon:
                                        Icon(state.obscure ? Icons.visibility_outlined : Icons.visibility_off_outlined),
                                    onPressed: () {
  ...
  @override
  FormBuilderFieldState<V5Text<T>, T> createState() => _V5TextState();
}

class _V5TextState<T> extends FormBuilderFieldState<V5Text<T>, T> {
  ...
  void initState() {
    ...
    controller = widget.controller ?? TextEditingController(text: value?.toString() ?? '');
  ...
  void _handleControllerChanged() {
    ...
    if (controller.text != (value ?? '')) {
      didChange(
        cast<T>(
          valueTransformer<T>(widget.valTransformer ?? ((v) => cast<T>(v)), widget.mask)(controller.text),
        ),
      );
    }
  }
